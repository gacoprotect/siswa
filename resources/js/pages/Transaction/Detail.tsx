...
const Detail: React.FC<TransactionDetailProps> = ({ transaction, onClose }) => {
    ...
    // Extract VA info
    const getVaInfo = () => {
        const { va_numbers, permata_va_number } = transaction.payment_data;
        if (va_numbers?.length) {
            const va = va_numbers[0];
            return { bank: va.bank.toUpperCase(), number: va.va_number };
        } else if (permata_va_number) {
            return { bank: 'Permata', number: permata_va_number };
        }
        return null;
    };
                ...
                >
                    ...
                    <div className="max-h-[85vh] overflow-y-auto px-6 py-8">
                        ...
                        <div className="mb-8 flex flex-col items-center text-center">
                            ...
                            <span
                                className={`$ transaction.status === 'pending' ? 'bg-yellow-100 text-yellow-800' : transaction.status === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' mt-3 inline-flex items-center rounded-full px-4 py-1 text-sm font-medium`}
                            >
                            ...
                            <div className="space-y-3">
                                ...
                                {vaInfo && (
                                    <>
                                        ...
                                        <DetailRow icon={<CreditCard size={16} />} label="Virtual Account" value={vaInfo.number} copyable />
                                    </>
                                )}
                                ...
                                <div className="mt-4 flex justify-between border-t border-gray-200 pt-3">
                                    ...
                                    <span className="text-lg font-semibold">{formatCurrency(transaction.payment_data.gross_amount)}</span>
                                </div>
                            </div>
                        </div>
